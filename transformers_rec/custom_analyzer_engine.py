...
class CustomAnalyzerEngine:
    ...
    def analyze(
        ...
        to return an identified entity, or a dict of minimum values
    ...
    def __remove_low_scores(
        ...
        score_threshold: Union[float, Dict[str, float]] = None,
    ) -> List[RecognizerResult]:
        """
        ...
        if isinstance(score_threshold, dict):
            ...
            for result in results:
                if (
                    result.entity_type in score_threshold
                    and result.score >= score_threshold[result.entity_type]
                ):
